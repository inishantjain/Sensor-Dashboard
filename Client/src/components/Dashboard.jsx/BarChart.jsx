...
function BarChart() {
  ...
  const [colors,setColors] = useState(Array.from({length: 10}, getRandomBrightColor));
  ...
  return (
    <StyledBarContainer>
      ...
      <div>
        ...
        <button onClick={()=>setStacked(prev=>!prev)} type="button">{isStacked ? "Stacked" : "Normal" }</button>
      </div>
      <Bar
        options={options}
        data={{
          ...
          datasets: fieldsToShow?.map((dataset_name, i) => ({
            ...
            stack: isStacked ? `Stack ${i}` : `Stack 0`,
          })),
        }}
      />
    </StyledBarContainer>
